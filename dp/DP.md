...
###### 그러나, 특히 이 문제의 경우 __당장 이전의 중복만 피하면 된다는 생각__을 바탕으로 그리디로 푼다면 1번 집 최소, 2번 집 최소의 경우를 선택하고 3번 집 최소를 선택하게 되는데 이것보다 1번 집 최소, 2번집 2번째로 최소, 3번집 최소 등의 경우의 수를 선택하면 더 최소인 결과값을 얻을 수 있게 된다.

###### 따라서 3단 리스트가 자칫 복잡해보여도 DP임을 파악한다면 주저 없이 도전해보자.
